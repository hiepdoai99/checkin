<template>
    <modal id="leave-create-edit-modal"
        ...
        <form
            ...
            <app-form-group type="date" v-model="formData.date" :label="$t('date')" :required="true"
                :error-message="$errorMessage(errors, 'date')" :placeholder="$t('enter_date')" />

            <div class="row align-items-center mb-3">
                <div class="col-md-6">
                    <app-form-group type="time" v-model="formData.start_time" :label="$t('late')" :required="true"
                        :error-message="$errorMessage(errors, 'start_time')" :placeholder="$t('enter_start_time')" />
                </div>
                <div class="col-md-6">
                    <app-form-group type="time" v-model="formData.end_time" :label="$t('back_early')" :required="true"
                        :error-message="$errorMessage(errors, 'end_time')" :placeholder="$t('enter_end_time')" />
                </div>
            </div>
...
export default {
    ...
    computed: {
        ...
        modalTitle(){
            return this.$can('assign_leaves') ? this.$t('late_leave_early') : this.$t('late_leave_early') ;
        }
    },
    watch: {
        employeeId: {
