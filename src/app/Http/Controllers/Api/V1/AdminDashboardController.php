...
//            ->whereDate('in_date', '=', todayFromApp())->with('user')->get();
            ->whereDate('in_date', '=',todayFromApp())->with('user')->get();

        $attendancesStats = $attendances->countBy(fn(Attendance $attendance) => $attendance->behavior);

        $leave = Leave::query()->filters($this->dashboardWhereHasUserAccessQueryFilter)
            ->whereDate('date', '=', todayFromApp())->where('status_id','!=',resolve(StatusRepository::class)->leaveApproved())->with('user')->get();
        $attendancesStats = $attendances->countBy(fn(Attendance $attendance) => $attendance->behavior);

        $notAttended = User::filters($this->userFilter)
            ->select(['id', 'first_name', 'last_name'])
            ->whereDoesntHave('attendances',function ($query){
                return $query->whereDate('in_date',todayFromApp());
            })->with('departments')->get();

        $attendanceApprove = resolve(StatusRepository::class)->attendanceApprove();

        $early = User::filters($this->userFilter)
            ->select(['id', 'first_name', 'last_name'])
            ->whereHas('attendances',function ($query){
                $query->whereDate('in_date',todayFromApp())
                    ->whereHas('details',function ($query){
                        return $query->where('out_time_early','>',0);
                    });
            })->with('departments')->get();
