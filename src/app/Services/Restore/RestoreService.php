...
class RestoreService extends TenantService
{
    ...
    {
        return $this->carbon($dateTime, 'Asia/Ho_Chi_Minh')
            ->parse()
            ->setTimezone('UTC')
            ->format('H:i');
    }
...
//        });
    ...
    public function saveAttendance($attendance): self
    {
        ...
        $inTime = $this->carbon($attendance->in_time, 'Asia/Ho_Chi_Minh')->parse()->addHours($attendance->in_offset);
        $outTime = $this->carbon($attendance->out_time, 'Asia/Ho_Chi_Minh')->parse()->addHours($attendance->out_offset);
    ...
    public function saveLeave($leave): self
    {
        ...
        if ($leave->total_days < 1) {
            $start_at = $this->carbon($leave->from_date . ' ' . $leave->from_time, 'Asia/Ho_Chi_Minh')
                ->parse()->setTimezone('UTC');
            $end_at = $this->carbon($leave->to_date . ' ' . $leave->to_time, 'Asia/Ho_Chi_Minh')
                ->parse()->setTimezone('UTC');
        } else {
