...
class AttendanceService extends TenantService
{
    ...
    public function getOutTime(): Carbon
    {
        return $this->getAttr('out_time') ? $this->carbon($this->getAttr('out_time'))->parse() : nowFromApp();
    }
    ...
    public function getBehavior(WorkingShiftDetails $details): string
    {
        ...
        $workShiftStartTime = $this->carbon($this->getToday()->toDateString().' '.$details->start_at, 'UTC')->parse();
        $this->getInTimeLate($workShiftStartTime);

        if ($this->getNow()->isBefore($workShiftStartTime)) {
            return 'early';
        }

        if ($this->getNow()->isAfter($workShiftStartTime->addMinutes($setting))) {
            return 'late';
        }
    ...
    public function getInTimeLate(Carbon $workShiftStartTime)
    {
        $lunchTime = $this->getLunchTime();
        $inTime = $this->carbon($this->details->in_time, 'UTC')->parse();

        $this->details->in_time_late = 0;

        if ($inTime->isBefore($lunchTime['start'])) {
            $this->details->in_time_late = $workShiftStartTime->diffInMinutes($inTime, false);
        }
        if ($inTime->isAfter($lunchTime['end'])) {
            $this->details->in_time_late = $lunchTime['end']->diffInMinutes($inTime, false);
        }
//         Log::info('getInTimeLate AID__'.$this->details->attendance_id.' __ '. $this->details->in_time_late );

        return $this;
    }

    public function getOutTimeEarly(Carbon $workShiftEndTime)
    {
        $lunchTime = $this->getLunchTime();
        $outTime = $this->carbon($this->details->out_time, 'UTC')->parse();
        $this->details->out_time_early = 0;

        if ($outTime->isBefore($lunchTime['start'])) {
            $this->details->out_time_early = $this->getOutTime()->diffInMinutes($lunchTime['start'], false);
        }
        if ($outTime->isAfter($lunchTime['end'])) {
            $this->details->out_time_early = $this->getOutTime()->diffInMinutes($workShiftEndTime, false);
        }

        // Log::info('getOutTimeEarly AID__'.$this->details->attendance_id.' __ '. $this->details->out_time_early. ' = '.$this->getOutTime()->toDateTimeString().' - '.$workShiftEndTime->toDateTimeString());
        // Log::error(print_r($this->workShiftDetails->attributesToArray(), true));
    ...
    {
        return [
            'start' => $this->carbon($this->getToday()->toDateString().' 05:00:00', 'UTC')->parse(),
            'end' => $this->carbon($this->getToday()->toDateString().' 06:30:00', 'UTC')->parse(),
        ];
    }
    ...
    {
        $lunchTime = $this->getLunchTime();
        if (is_null($this->workShiftDetails)) {
            $this->getPunchInDate('manual');
        }
        $works = 0.0;
        $inTime = $this->carbon($this->details->in_time, 'UTC')->parse();
        $outTime = $this->carbon($this->details->out_time, 'UTC')->parse();
        $workShiftStartTime = $this->carbon($this->getToday()->toDateString().' '.$this->workShiftDetails->start_at, 'UTC')->parse();
        $workShiftEndTime = $this->carbon($this->getToday()->toDateString().' '.$this->workShiftDetails->end_at, 'UTC')->parse();

        if ($inTime->isBefore($lunchTime['start'])) {
            $works += 0.5;
        }

        if ($inTime->isBefore($workShiftEndTime) && $outTime->isAfter($lunchTime['end'])){
            $works += 0.5;
        }

        if (empty($this->details->works)) {
            $this->details->works = $works;
        } else {
            if ($this->details->works < $works) {
                $this->details->works = $works;
            }
        }
    ...
    public function manualAddPunchHanet()
    {
        $this->setAttr('today', $this->getAttr('in_date'));

        $attendanceDetails = $this->getExistingPunchTime();
        ...
        if (!$attendanceDetails) {
            ...
            // Log::info(print_r($this->details->attributesToArray(), true));
            return $this;
        }

        // TODO kiem tra in_time co som hon $attendanceDetails->in_time khong

        $this->details = $attendanceDetails;
        $this->getPunchInDate('manual');
        $this->getOutTimeEarly(
            $this->carbon($this->getToday()->toDateString().' '.$this->workShiftDetails->end_at, 'UTC')->parse()
        );
        $details = $this->getWorks();
        $details->fill([
            'out_time' => $this->carbon($this->getAttr('out_time'))->toDateTime(),
        ])->save();
        
        // Log::debug(print_r($this->details->attributesToArray(), true));

        // $this->sendNotification($this->details->load('status'));

        return $this;
    }

    public function getExistingPunchTime()
    {
        $inTime = $this->carbon($this->getAttr('in_time'))->toDateTime();
        $inDate = $this->carbon($this->getAttr('in_time'))->toDate();

        $statuses = resolve(StatusRepository::class)->attendanceApprovePending();

        return AttendanceDetails::query()
            ->whereHas('attendance',
                function(Builder $builder) use ($inDate) {
                    $builder->where('user_id', $this->model->id)
    ...
    public function validateOwnDepartmentUser(): AttendanceService
    {
