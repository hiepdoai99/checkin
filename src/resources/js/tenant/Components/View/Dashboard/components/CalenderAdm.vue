<template>
  <div class="card card-with-shadow border-0 h-100 min-height-full">
    <div class="card-header bg-transparent p-primary d-flex justify-content-start align-items-center">
      <app-icon name="calendar" stroke="#89C111" />
      <h5 class="card-title text-capitalize mb-0">
        {{ $t('Lịch') }}
      </h5>
    </div>
    <app-calendar :preloader="dataLoader" :options="calendarOptions" />
    <app-calender-modal v-if="isModalActive" v-model="isModalActive" :eventData="eventData" :selected-url="selectedUrl"
  ...
  methods: {
    ...
    selectedEvent(arg) {
      ...
      if (arg.event.id && isSameOrAfterThisYear(arg.event.start)) {
        this.isModalActive = true;
        this.selectedUrl = `${EVENTS}/${arg.event.id}`;
      } else {
        if (!arg.event.id) {
