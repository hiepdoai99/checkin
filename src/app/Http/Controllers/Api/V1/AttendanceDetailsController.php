...
use App\Actions\MobilePunchInWebhook;
...
use App\Http\Requests\Api\MobileCamRequest;
...
{
    use DateRangeHelper, AssignRelationshipToPaginate, DateTimeHelper,FileHandler;
    ...
    public function mobileCam(MobileCamRequest $mobileCamRequest)
    {
        // Lấy dữ liệu từ request
        $file = $mobileCamRequest->file('image');
        // Lưu file vào storage
        $path = Storage::putFile('punch-in-images', $file);
        // Trả về đường dẫn file đã lưu
//        return ['path' => $path];
        dd(['path' => $path]);
        if ( app(MobilePunchInWebhook::class)->execute($mobileCamRequest))
        {
            return $this->respondSuccess('Chấm Công thành công');
        }
